...
class VO {
    ...
    function configuracao() {
        ...
        $this->extensoesArquivos = array("pdf", "doc", "docx", "xls", "xlsx", "jpg", "jpeg");
        $this->datas = array();
        $this->horas = array();
    ...
    function PreencheVOArquivos($files) {
        ...
        while (list($key, $val) = each($files)) {
            ...
            if ($val['tmp_name']) {
                ...
                if ($this->$key) {
                    //se tiver valor no campo e tiver tmp_name, apagar o arquivo anterior
                    $this->apagarArquivoUnico($key, $this->$key);
                }

                if ($this->isImagem($key)) {
    ...
    function verificarExtensoesArquivos() {
        $campos = $this->arquivos;
        $obrigatorio = '';
        ...
        while (list($key, $val) = each($campos)) {
            ...
            $ext = strtok(".");
            if ($this->$val) {
                !$this->validarExtensao($ext, $val) ? $this->validar[$val] = 'Formato de arquivo inválido.' : false;
            }
        }
        return $obrigatorio;
    }
    ...
    function verificarEmail($valor) {
        $valor && @!ereg("^([0-9,a-z,A-Z]+)([.,_]([0-9,a-z,A-Z]+))*[@]([0-9,a-z,A-Z]+)([.,_,-]([0-9,a-z,A-Z]+))*[.]([0-9,a-z,A-Z]){2}([0-9,a-z,A-Z])?$", $valor) ? $aux = 'E-mail Inválido' : false;
    ...
    function validar() {
        ...
        !$this->validar ? $this->verificarExtensoesArquivos() : false;
    }
    ...
    function verificarDiasUteis($data, $dias) {

        $dt_fim = $this->somarDiasUteis($data, $dias);

        $dataFim = new DateTime(implode("-", array_reverse(explode("/", $dt_fim))));
    ...
    function verificarFeriado() {
        return $this->repositorio->verificarFeriado($this);
    }

    function somarDiasUteis($str_data, $int_qtd_dias_somar) {
        ...
        while ($int_qtd_dias_uteis < $int_qtd_dias_somar) {
            ...
            if (($dias_da_semana = gmdate('w', strtotime('+' . $count_days . ' day', mktime(0, 0, 0, $array_data[1], $array_data[2], $array_data[0])))) != '0' && $dias_da_semana != '6') {
                ...
                //Verifica no Banco se a data é Feriado
                $this->DT_DIA_FERIADO = gmdate('d/m/Y', strtotime('+' . $count_days . ' day', strtotime($str_data)));
                $feriado = $this->verificarFeriado();
    ...
    function formatarCPF($cpf) {
        ...
        $valor = substr($cpf, 0, 3) . "." . substr($cpf, 3, 3) . "." . substr($cpf, 6, 3) . "-" . substr($cpf, 9, 2);
    ...
    function erroOracle($arrayErro) {
        ...
        } else if ($arrayErro['code'] >= 20000) {

            $posicao = mb_stripos($arrayErro['message'], ":");
            $string1 = mb_substr($arrayErro['message'], $posicao + 1);
            $posicao2 = mb_stripos($arrayErro['message'], "ORA", $posicao);
            $erro = trim(mb_substr($arrayErro['message'], $posicao + 1, ($posicao2 - $posicao) - 1));

        } else {
            $erro = 'Erro: ' . str_replace('"', '', $arrayErro['message']);
        }
